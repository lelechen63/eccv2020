...
def define_G(input_nc, output_nc, ngf, netG, n_downsample_global=3, n_blocks_global=9, n_local_enhancers=1,
    ...
    if netG == 'global':
        netG = GlobalGenerator1(input_nc, output_nc, ngf,
...
class GlobalGenerator1(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,  pad_type='reflect'):
        ...
        model = []
        model = [nn.ReflectionPad2d(3), nn.Conv2d(
            3, ngf, kernel_size=7, padding=0), norm_layer(ngf), activation]
        # downsample
        model += [nn.Conv2d(ngf, ngf * 2, kernel_size=3, stride=2, padding=1),   # 128, 128, 128
                  norm_layer(ngf * 2), activation]
        ...
        for i in range(n_blocks):
            model += [ResnetBlock(ngf * 8, padding_type=pad_type,
...
class GlobalGenerator1_kou(nn.Module):
     # the most simple network, the input is I_0 + L_0 + L_i, the output is I_i. We concate all inputs in channel and encode, decode it.
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,  pad_type='reflect', pretrain = True):
        super(GlobalGenerator1, self).__init__()
        activation = nn.ReLU(True)
        model = [nn.ReflectionPad2d(3), nn.Conv2d(
            input_nc, ngf, kernel_size=7, padding=0), norm_layer(ngf), activation]
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, getIntermFeat=False):
        ...
        sequence = [[nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2,
                               padding=padw), nn.LeakyReLU(0.2, True)]]  # 64, 128
