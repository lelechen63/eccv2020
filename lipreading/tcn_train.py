...
def test(model, net):
    ...
    # with torch.no_grad():
        dataset = MyDataset(opt.data_root,           
            opt.txt_padding,
            'test')
            
        print('num_test_data:{}'.format(len(dataset.datalist)))  
        model.eval()
        loader = dataset2dataloader(dataset, shuffle=True)
        loss_list = []
        wer = []
        cer = []
        crit = nn.CTCLoss().cuda(device=opt.cuda1)
        tic = time.time()
        for (i_iter, input) in enumerate(loader):  
            if i_iter == 20:
                break      
            vid = input.get('vid').cuda(device=opt.cuda1)
            txt = input.get('txt').cuda()
            vid_len = input.get('vid_len').cuda(device=opt.cuda1)
            txt_len = input.get('txt_len').cuda(device=opt.cuda1)
...
def train(model, net):
    ...
    print('num_train_data:{}'.format(len(dataset.datalist)))    
    cross_entropy_loss  = nn.CrossEntropyLoss().cuda(device=opt.cuda1)
    tic = time.time()
    ...
    for epoch in range(opt.max_epoch):
        ...
        for (i_iter, input) in enumerate(loader):
            ...
            model.train()
            vid = input.get('vid').cuda(device=opt.cuda1)
            txt = input.get('txt').cuda(device=opt.cuda1)
            vid_len = input.get('vid_len').cuda(device=opt.cuda1)
            txt_len = input.get('txt_len').cuda(device=opt.cuda1)
...
if(__name__ == '__main__'):
    ...
    model = LipNet()
    device_ids = [int(i) for i in opt.device_ids.split(',')]
