...
def landmark_extractor():
    ...
    root_path = '/home/cxu-serve/p1/common/faceforensics/original_sequences/youtube/cropped'
    train_list = sorted(os.listdir(os.path.join(root_path, 'videos')))
    batch_length =  int( len(train_list))
    for i in tqdm(range(batch_length * (config.batch_id -1), batch_length * (config.batch_id ))):
        p_id = train_list[i]
        cropped_video_path = os.path.join(root_path,  'videos',  p_id)
        
        lmark_path = os.path.join(root_path,  'videos',  p_id[:-3] + 'npy')
            
        if os.path.exists(lmark_path):
            continue
        try:
            lmark = []
            for img_path in sorted(os.listdir(os.path.join(root_path,  'images',  p_id[:3])  )):                
                frame = cv2.imread(os.path.join(root_path,  'images',  p_id[:3], img_path))
...
def RT_compute():
    ...
    for i in tqdm(range(batch_length)):
        p_id = train_list[i]
        lmark_path = os.path.join(root_path ,"videos", p_id)
        if lmark_path[-3:] !=  'npy':
            continue
        rt_path = os.path.join( root_path,"videos" , p_id[:-4] +'_rt.npy')
        front_path = os.path.join(  root_path, "videos" , p_id[:-4] +'_front.npy')
